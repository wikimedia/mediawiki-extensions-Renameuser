...
class RenameUserCleanup extends Maintenance {
	public function __construct() {
		parent::__construct();
		$this->mDescription =
			'Maintenance script to finish incomplete rename user, in particular to reassign edits that were missed';
	...
	public function checkUserExistence( $olduser, $newuser ) {
		...
		if ( $olduser->getId() ) {
			...
			$this->output( 'proceed anyways? We\'ll only re-attribute edits that have the new user uid (or 0)' );
			$this->output( ' or the uid specified by the caller, and the old user name.  [N/y]   ' );
